...
If your npm is version **_7 or higher_**, do:
...
Windows is a bit trickier, since `npm` might be installed in the "Program Files" directory, which needs admin privileges to modify current Windows. Therefore, run the following commands **inside a `cmd.exe` started with "Run as Administrator"**:
...
If your npm version is **_7 or higher_**, do:
