...
describe('find-visualstudio', function () {
  ...
  it('error from PowerShell', async function () {
    ...
    assert.ok(
      /use PowerShell/i.test(finder.errorLog[0]),
      `expect error, output: ${finder.errorLog[0]}`
    );
  ...
  it('wrong JSON from PowerShell', async function () {
    ...
    assert.ok(
      /expected array/i.test(finder.errorLog[0]),
      `expect error, output: ${finder.errorLog[0]}`
    );
    assert.equal(vsInfo, null);
  });
  ...
  it('VS2022 Preview with C++ workload', async function () {
    ...
    finder.findNewVSUsingSetupModule = async () => null;
    finder.findVisualStudio2019OrNewer = async () => {
      const file = path.join(__dirname, 'fixtures', 'VS_2022_Community_workload.txt');
      const data = fs.readFileSync(file);
      const parsedData = finder.parseData(null, data, '', { checkIsArray: true });
      return finder.processData(parsedData, [2019, 2022]);
    };
  ...
  async function verifyVSSetupData(
    fixtureName,
    vsType,
    vsYear,
    sdkVersion,
    ...
    assert.deepStrictEqual(info, {
      msBuild: msBuildPath,
      path: `C:\\${expectedProgramFilesPath}\\Microsoft Visual Studio\\${vsYear}\\${vsType}`,
      sdk: sdkVersion,
  ...
  function allVsVersions(finder) {
    ...
    finder.findVisualStudio2017 = async () => {
      ...
      const data1 = JSON.parse(
        fs.readFileSync(path.join(__dirname, 'fixtures', 'VS_2017_BuildTools_minimal.txt'))
      );
      const data2 = JSON.parse(
        fs.readFileSync(path.join(__dirname, 'fixtures', 'VS_2017_Community_workload.txt'))
      );
    ...
    finder.findVisualStudio2019OrNewer = async () => {
      ...
      const data1 = JSON.parse(
        fs.readFileSync(path.join(__dirname, 'fixtures', 'VS_2019_BuildTools_minimal.txt'))
      );
      const data2 = JSON.parse(
        fs.readFileSync(path.join(__dirname, 'fixtures', 'VS_2019_Community_workload.txt'))
      );
      const data3 = JSON.parse(
        fs.readFileSync(path.join(__dirname, 'fixtures', 'VS_2022_Community_workload.txt'))
      );
      const data = JSON.stringify(data0.concat(data1, data2, data3));
      const parsedData = finder.parseData(null, data, '', { checkIsArray: true });
      return finder.processData(parsedData, [2019, 2022]);
