...
describe('find-node-directory', function () {
  ...
  it('test find-node-directory - node build', function () {
    for (let next = 0; next < platforms.length; next++) {
      ...
      if (platforms[next] === 'win32') {
        assert.strictEqual(
          findNodeDirectory('/y/node_modules/npm/node_modules/node-gyp/lib', processObj),
          path.join('/y')
        );
      } else {
        assert.strictEqual(
          findNodeDirectory('/y/lib/node_modules/npm/node_modules/node-gyp/lib', processObj),
          path.join('/y')
        );
      }
    }
  ...
  it('test find-node-directory - node install', function () {
    for (let next = 0; next < platforms.length; next++) {
      const processObj = { execPath: '/x/y/bin/node', platform: platforms[next] };
      assert.strictEqual(
        findNodeDirectory('/x/y/z/a/b/c/deps/npm/node_modules/node-gyp/lib', processObj),
