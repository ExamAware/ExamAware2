...
export class PluginHost extends EventEmitter {
  ...
  list(): PluginListItem[] {
    return Array.from(this.records.values()).map((record) => ({
      ...
      rendererWindows: record.manifest.activation.rendererWindows,
  ...
  async loadPlugin(record: InternalPluginRecord) {
    ...
    try {
      ...
      const legacyContext = this.createRuntimeContext(record)
      const entryApiVersion = (factory as { apiVersion?: number }).apiVersion
      if (entryApiVersion === 2 && manifestApiVersion(record.manifest) !== 2) {
        throw new Error(`插件 ${record.name} 使用 API V2 入口，但 manifest 未声明 apiVersion: 2`)
      }
      const entryProcess = (factory as { process?: string }).process
      if (entryProcess === 'renderer') {
        throw new Error(`插件 ${record.name} 的 main 入口导出了 renderer 插件定义`)
      }
      if (record.manifest.apiVersion === 2 && (entryApiVersion !== 2 || entryProcess !== 'main')) {
        throw new Error(
          `插件 ${record.name} 的 manifest 声明 API V2，但 main 入口不是 defineMainPlugin`
        )
      }
      const runtimeContext =
        record.manifest.apiVersion === 2
          ? await createMainPluginContext({
              manifest: record.manifest,
              runtime: legacyContext,
              listPlugins: () => this.list(),
              setEnabled: (name, enabled) => this.setEnabled(name, enabled),
              reload: (name) => this.reloadPlugin(name)
            })
          : legacyContext
      const cleanup = await factory(runtimeContext, this.getConfig(record.name))
   ...
   */
  setupIpcChannels() {
    const ipc = new IpcRegistrar(this.options.ctx)
    ipc.handle(ipcChannels.plugins.list, async () => this.list())
    ipc.handle(ipcChannels.plugins.toggle, async (_event, name, enabled) => {
      await this.setEnabled(name, enabled)
      return this.list()
    })
    ipc.handle(ipcChannels.plugins.reload, async (_event, name) => {
      await this.reloadPlugin(name)
      return this.list()
    })
    ipc.handle(ipcChannels.plugins.uninstall, async (_event, name) => {
      await this.uninstallPlugin(name)
      return this.list()
    })
    ipc.handle(ipcChannels.plugins.services, async () => this.getServiceSnapshot())
    ipc.handle(ipcChannels.plugins.service, async (_event, name, owner) =>
      this.peekServiceValue(name, owner)
    )
    ipc.handle(ipcChannels.plugins.getConfig, async (_event, name) => this.getConfig(name))
    ipc.handle(ipcChannels.plugins.setConfig, async (_event, name, config) => {
      await this.replaceConfig(name, config ?? {})
      return this.getConfig(name)
    })
    ipc.handle(ipcChannels.plugins.patchConfig, async (_event, name, partial) => {
      await this.patchConfig(name, partial ?? {})
      return this.getConfig(name)
    })
    ipc.handle(ipcChannels.plugins.rendererEntry, async (_event, name) =>
      this.getRendererEntryUrl(name)
    )
    ipc.handle(ipcChannels.plugins.readme, async (_event, name) => this.getReadme(name))
    ipc.handle(ipcChannels.plugins.fetchSource, async (_event, payload) =>
    ...
    ipc.handle(ipcChannels.plugins.registryReadme, async (_event, payload) => {
      const pkg = payload?.pkg?.trim()
      if (!pkg) throw new Error('缺少包名')
    ...
    ipc.handle(ipcChannels.plugins.installPackage, async (_event, filePath) => {
      const target = await this.installPackage(filePath)
      await this.scan()
      await this.loadAll()
      return { installedPath: target, list: this.list() }
    })
    ipc.handle(ipcChannels.plugins.installDirectory, async (_event, dirPath) => {
  ...
  private broadcastConfig(name: string, config: Record<string, any>) {
    for (const window of BrowserWindow.getAllWindows()) {
      try {
        sendIpcEvent(window.webContents, ipcChannels.plugins.configChanged, { name, config })
      } catch (error) {
  ...
  private broadcastRegistryProgress(progress: RegistryInstallProgress) {
    for (const window of BrowserWindow.getAllWindows()) {
      try {
        sendIpcEvent(window.webContents, ipcChannels.plugins.registryProgress, progress)
      } catch (error) {
  ...
  private broadcastState() {
    ...
    for (const window of BrowserWindow.getAllWindows()) {
      try {
        sendIpcEvent(window.webContents, ipcChannels.plugins.stateChanged, payload)
      } catch (error) {
  ...
  private createRuntimeContext(record: InternalPluginRecord): PluginRuntimeContext {
    ...
    const rpcChannels = pluginRpcChannels(record.name)
    const rpcClient = new JsonRpcClient({
      send: (message) => {
        ...
        sendIpcEvent(target.webContents, rpcChannels.toRenderer, message)
      },
      onMessage: (handler) => {
        const listener = (_event: Electron.IpcMainEvent, payload: string) => handler(payload)
        ipcMain.on(rpcChannels.toMain.channel, listener)
        const disposer = () => ipcMain.off(rpcChannels.toMain.channel, listener)
      ...
      onMessage: (handler) => {
        const listener = (event: Electron.IpcMainEvent, payload: string) =>
          handler(payload, (response) =>
            sendIpcEvent(event.sender, rpcChannels.toRenderer, response)
          )
        ipcMain.on(rpcChannels.toMain.channel, listener)
        const disposer = () => ipcMain.off(rpcChannels.toMain.channel, listener)
        record.group.add(disposer)
    ...
    return {
      ...
      windows: {
        broadcast: (channel: string, payload?: any) => {
          const endpoint = dynamicEventEndpoint<[payload?: any]>(channel)
          BrowserWindow.getAllWindows().forEach((w) => {
            try {
              sendIpcEvent(w.webContents, endpoint, payload)
            } catch (error) {
...
function manifestApiVersion(manifest: ResolvedPluginManifest) {
