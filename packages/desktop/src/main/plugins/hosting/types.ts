...
import type { RpcClientProxy, RpcMethodName, RpcServiceToken } from '@dsz-examaware/rpc'
...
export interface PluginRpcApi {
  get<TService extends object>(token: RpcServiceToken<TService>): RpcClientProxy<TService>
  get<T extends Record<string, any> = Record<string, any>>(token: string): T
  expose<TService extends object>(token: RpcServiceToken<TService>, service: TService): Disposer
  expose(token: string, service: Record<string, any>): Disposer
  notify<TService extends object, Method extends RpcMethodName<TService>>(
