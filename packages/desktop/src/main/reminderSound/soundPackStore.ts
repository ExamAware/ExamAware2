...
function requireBuffer(value: Buffer | undefined, message: string): Buffer {
  if (!value) throw new Error(`无效的 .ea2r 铃声包：${message}`)
  return value
}

const readShortText = (value: unknown, field: string, maxLength = 100): string => {
  if (typeof value !== 'string' || !value.trim() || value.length > maxLength) {
    return fail(`${field} 必须是 1 到 ${maxLength} 个字符的文本`)
  }
  return value.trim()
}

const validateEntryPath = (entryPath: unknown, field = '条目路径'): string => {
...
const validateSoundEntry = (
  ...
  assertRecord(value, `sounds.${kind} 必须是对象`)
  ...
  if (typeof sha256 !== 'string' || !SHA256_PATTERN.test(sha256)) {
    return fail(`sounds.${kind}.sha256 必须是 SHA-256 哈希`)
  }
  return { name, path, sha256: sha256.toLowerCase() }
}

const validateManifest = (value: unknown): ReminderSoundPackManifest => {
  assertRecord(value, 'manifest.json 必须是对象')
  ...
  const soundsValue = value.sounds
  assertRecord(soundsValue, 'sounds 必须包含 start、alert 和 end')
  const sounds = Object.fromEntries(
    REMINDER_SOUND_KINDS.map((kind) => [kind, validateSoundEntry(soundsValue[kind], kind)])
...
const validateFiles = (manifest: ReminderSoundPackManifest, files: Map<string, Buffer>) => {
  ...
  for (const kind of REMINDER_SOUND_KINDS) {
    ...
    const bytes = requireBuffer(files.get(sound.path), `缺少 ${kind} 声音文件 ${sound.path}`)
...
const parseArchive = async (filePath: string): Promise<ValidatedPack> => {
  ...
  const manifestBytes = requireBuffer(files.get('manifest.json'), '缺少 manifest.json')
