...
import type { ReminderSoundPackStore } from './soundPackStore'

type IpcHandler<Endpoint extends AnyIpcInvokeEndpoint> = (
  event: unknown,
  ...args: IpcEndpointArgs<Endpoint>
) => IpcEndpointResult<Endpoint> | Promise<IpcEndpointResult<Endpoint>>

interface ReminderSoundPackIpcOptions {
  handle<Endpoint extends AnyIpcInvokeEndpoint>(
    endpoint: Endpoint,
    listener: IpcHandler<Endpoint>
  ): void
...
export function registerReminderSoundPackIpc(options: ReminderSoundPackIpcOptions) {
  options.handle(ipcChannels.reminderSounds.list, () => options.store.list())
  options.handle(ipcChannels.reminderSounds.import, async () => {
