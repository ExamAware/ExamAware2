...
} from '../../../src/main/reminderSound/soundPackIntegration'
import {
...
describe('reminder sound pack IPC', () => {
  it('lists packages and imports only the file selected by the restricted dialog', async () => {
    ...
    registerReminderSoundPackIpc({
      handle: (endpoint, listener) => handlers.set(endpoint.channel, listener),
      showOpenDialog,
      store
    })

    await expect(handlers.get(ipcChannels.reminderSounds.list.channel)?.({})).resolves.toEqual([
      POND_REMINDER_SOUND_PACK,
      importedPack
    ])
    await expect(handlers.get(ipcChannels.reminderSounds.import.channel)?.({})).resolves.toEqual({
      canceled: false,
      pack: importedPack,
