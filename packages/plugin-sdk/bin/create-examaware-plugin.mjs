...
const templateRepo =
  process.env.EXAMAWARE_PLUGIN_TEMPLATE ||
  'https://github.com/ExamAware/examaware-plugin-template.git';
...
function cloneTemplate(repo, ref, destination) {
  ...
  if (result.status !== 0) {
    console.error(
      'Failed to clone template. Make sure git is installed and the repo is reachable.'
    );
